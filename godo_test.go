...
func TestNewRequest_PublicNetworking_NotPresent(t *testing.T) {
	c := NewClient(nil)

	inURL, outURL := "/foo", defaultBaseURL+"foo"
	inBody := &DropletCreateRequest{Name: "l"}
	outBody := `{"name":"l","region":"","size":"","image":0,` +
		`"ssh_keys":null,"backups":false,"ipv6":false,` +
		`"private_networking":false,"monitoring":false,"tags":null}` + "\n"
	req, _ := c.NewRequest(ctx, http.MethodPost, inURL, inBody)
...
func TestNewRequest_PublicNetworking_True(t *testing.T) {
	c := NewClient(nil)

	trueVal := true
	inURL, outURL := "/foo", defaultBaseURL+"foo"
	inBody := &DropletCreateRequest{Name: "l", PublicNetworking: &trueVal}
	outBody := `{"name":"l","region":"","size":"","image":0,` +
		`"ssh_keys":null,"backups":false,"ipv6":false,` +
		`"private_networking":false,"monitoring":false,"tags":null,"public_networking":true}` + "\n"
	req, _ := c.NewRequest(ctx, http.MethodPost, inURL, inBody)
...
func TestNewRequest_PublicNetworking_False(t *testing.T) {
	c := NewClient(nil)

	falseVal := false
	inURL, outURL := "/foo", defaultBaseURL+"foo"
	inBody := &DropletCreateRequest{Name: "l", PublicNetworking: &falseVal}
	outBody := `{"name":"l","region":"","size":"","image":0,` +
		`"ssh_keys":null,"backups":false,"ipv6":false,` +
		`"private_networking":false,"monitoring":false,"tags":null,"public_networking":false}` + "\n"
	req, _ := c.NewRequest(ctx, http.MethodPost, inURL, inBody)
