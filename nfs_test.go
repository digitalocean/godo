...
func TestNfsCreate(t *testing.T) {
	...
	mux.HandleFunc("/v2/nfs", func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Fprint(w, `{"share": {"id": "test-nfs-id", "name": "test-nfs-share", "size_gib": 50, "region": "atl1", "status": "PROVISIONING", "user_id": 123, "created_at":"2023-10-01T00:00:00Z", "vpc_ids": []}}`)
	})
...
func TestNfsList(t *testing.T) {
	...
	mux.HandleFunc("/v2/nfs", func(w http.ResponseWriter, r *http.Request) {
		...
		if page == "2" {
			w.WriteHeader(http.StatusOK)
			fmt.Fprint(w, `{"shares": [{"id": "test-nfs-id-2", "name": "test-nfs-share-2", "size_gib": 50, "region": "atl1", "status": "PROVISIONING", "user_id": 123, "created_at":"2023-10-01T00:00:00Z", "vpc_ids": []}]}`)
		} else {
			w.WriteHeader(http.StatusOK)
			fmt.Fprint(w, `{"shares": [{"id": "test-nfs-id-1", "name": "test-nfs-share-1", "size_gib": 50, "region": "atl1", "status": "PROVISIONING", "user_id": 123, "created_at":"2023-10-01T00:00:00Z", "vpc_ids": []}]}`)
		}
	})
